 ...
 * Fungsi: Memproses transaksi stok keluar (support multiple items)
 */
...
if (empty($tanggal) || empty($penanggung_jawab)) {
    header('Location: stok_keluar.php?status=error&msg=Tanggal dan Penanggung Jawab wajib diisi!');
    exit;
}
...
if (empty($items) || count($items) == 0) {
    header('Location: stok_keluar.php?status=error&msg=Minimal harus ada 1 barang!');
    exit;
}
...
foreach ($items as $item) {
    ...
    if (in_array($barang_id, $barang_ids)) {
        header('Location: stok_keluar.php?status=error&msg=Tidak boleh ada barang yang sama dalam satu transaksi!');
        exit;
    }
...
if (empty($validated_items)) {
    header('Location: stok_keluar.php?status=error&msg=Tidak ada barang yang valid!');
    exit;
}
